...
def get_conditions(filters):
	...
	if filters.get("from_date") and filters.get("to_date"):
		...
		conditions.append(f" so.transaction_date between '{from_date}' and '{to_date}' ")
	...
	if filters.get("from_time") and filters.get("to_time"):
		...
		conditions.append(f" DATE_FORMAT(so.order_time, '%T') between '{from_time}' and '{to_time}' ")
...
def get_columns(filters):
	columns = [
		...
		{"label": _("Order Time"), "fieldname": "order_time", "fieldtype": "Time", "width": 90},
		{
			"label": _("Sales Order"),
